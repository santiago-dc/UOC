...
### Respuesta 1.1:
Hemos elegido el dataset https://www.kaggle.com/vjchoudhary7/customer-segmentation-tutorial-in-python para clasificación de clientes de un centro comercial, posee variables categóricas y variables numéricas.
```{r message= FALSE, warning=FALSE}
customers_data<-read.csv("Mall_Customers.csv", header=T, sep=",")
attach(customers_data)
colnames(customers_data) <- c("CustomerID", "Gender", "Age", "AnnualIncome", "SpendingScore")
summary(customers_data)
```
Comprobamos valores nulos. No hay ninguno, el dataset parece estar ya tratado.
```{r message= FALSE, warning=FALSE}
colSums(is.na(customers_data))
colSums(customers_data==" ")
colSums(customers_data==0)#Dada la naturaleza de los atributos, aquí el 0 no es un valor admisible, por ello lo busco para asegurarme de que no hay ningún valor numérico nulo

```

Customer ID no es relevante para el modelo, no nos aponta ninguna información así que lo quitamos, tampoco gender por ser un atributo categórico.
```{r message= FALSE, warning=FALSE}
x<-customers_data[,3:5]
str(x)
```
Normalizamos
```{r message= FALSE, warning=FALSE}
...
```
Aplicamos el algoritmo kmeans, como desconocemos el número de clusters idóneo probamos con múltimples, según la fuente, el valor idóneo está en 5 clusters.
```{r message= FALSE, warning=FALSE}
d <- daisy(x) 
resultados <- rep(0, 10)
for (i in c(2,3,4,5,6,7,8,9,10))
...
```
Posteriormente mostramos los valores de la silueta media para determinar qué número de clusters es el más adecuado.
```{r message= FALSE, warning=FALSE}
plot(2:10,resultados[2:10],type="o",col="blue",pch=0,xlab="Numero de clusters",ylab="Silueta")
```

Comprobamos lo mismo aplicando como criterio el método elbow
```{r message= FALSE, warning=FALSE}
resultados <- rep(0, 10)
for (i in c(2,3,4,5,6,7,8,9,10))
{
...
Comprobamos también el valor del número de clústers que mejor funcione de acuerdo a el crtiterio de la silueta media (“asw”) y el de Calinski-Harabasz (“ch”).
```{r message= FALSE, warning=FALSE}
library(fpc)
fit_ch  <- kmeansruns(x, krange = 1:10, criterion = "ch") 
fit_asw <- kmeansruns(x, krange = 1:10, criterion = "asw") 

#cat("silueta media: ",fit_asw$bestk)
plot(1:10,fit_asw$crit,type="o",col="blue",pch=0,xlab="Número de clústers",ylab="Criterio Calinski-Harabasz")
```

De nuevo el 5, aunque aquí el 6 le supera por poco.
```{r message= FALSE, warning=FALSE}
#cat("Calinski-Harabasz ",fit_ch$bestk)
plot(1:10,fit_ch$crit,type="o",col="blue",pch=0,xlab="Número de clústers",ylab="Criterio Calinski-Harabasz")
```

Esta última hace dudar de si 5 es el valor adecuado, parece que 6 también es candidatos a ser le valor del número de clusters.

Como se puede observar, el número de clusters más viables son 5 o 6, pero no está claro. Comprobamos ambos para ver si alguno de los resultados nos satisface. 

Para 5 clusters:
```{r message= FALSE, warning=FALSE}
customers5clusters <- kmeans(x, 5)
# Annual Income y Age
plot(x[c(1,2)], col=customers5clusters$cluster)
```
```{r message= FALSE, warning=FALSE}
# SpendingScore y AnnualIncome
plot(x[c(2,3)], col=customers5clusters$cluster)
```

Para 6 clusters:

```{r message= FALSE, warning=FALSE}
customers6clusters <- kmeans(x, 6)
# AnnualIncome y Age
plot(x[c(1,2)], col=customers6clusters$cluster)
```

```{r message= FALSE, warning=FALSE}
customers6clusters <- kmeans(x, 6)
# SpendingScore y AnnualIncome
plot(x[c(2,3)], col=customers6clusters$cluster)
```

Ahora vamos a evaluar la calidad del proceso de agregación. Para ello usaremos la función silhouette que calcula la silueta de cada muestra
```{r message= FALSE, warning=FALSE}
d  <- daisy(x) 
sk5 <- silhouette(customers5clusters$cluster, d)
sk6 <- silhouette(customers6clusters$cluster, d)
aux7 <- kmeans(x, 7)
sk7 <- silhouette(aux7$cluster, d)
aux4 <- kmeans(x, 4)
sk4 <- silhouette(aux4$cluster, d)
...
Usando silhouette y gracias a las imagenes anteriores, concluimos que 5 clusters es el número más adecuado, es cierto que para 6 clusters el valor es un poco más alto, pero las anteriores gráficas no dejan duda de que 5 es mejor opción que 6. Parece ser que SpendingScore y AnnualIncome son las variables que nos son más útiles para representar la agrupación.

Volvamos entonces a los datos ya agrupados en 5 grupos.
```{r message= FALSE, warning=FALSE}
par(mfcol=c(1,2))
plot(x[c(1,2)], col=customers5clusters$cluster)
plot(x[c(2,3)], col=customers5clusters$cluster)
...
Los grupos se pueden interpretar como (de arriba a abajo):

* Grupo 1:  es el grupo que más gasta pese a su bajo nivel adquisitivo, esto se explica debido a que todos son gente joven, y probablemente el dinero que gastan no es suyo, si no de sus padres, en muchos casos. Posible nombre: Grupo bajo-pródigo.
* Grupo 2: tiene el mismo poder adquisitivo que el grupo 1, sin embargo los gastos están acorde con sus ingresos. Posible nombre: Grupo bajo-sensato.
* Grupo 3: el grupo 3 es el análogo al grupo 2, tiene más poder adquisitivo y por lo tanto gasta algo más. Posible nombre: Grupo medio.
* Grupo 4: el grupo 4 es el que más gasta junto al grupo 1, pero tiene un poder adquisitivo acorde. Presenta la misma proporcionalidad que los grupos 2 y 3. Posible nombre: Grupo de derrochadores.
* Grupo 5: el grupo 5 podría catalogarse como el de los ahorradores, tiene un poder adquisitivo un poco menor que el 4 pero gasta incluso menos que el 2. Posible nombre: Grupo de ahorradores.
...
* Hierarchical Clustering: Este método realiza clustering no supervisado, se diferencia del kmeans en que no es necesario indicar el número de clusters, el propio algoritmo lo indica. El algoritmo selecciona cada dato como un centroide y lo agrupa con el centroide más cercano según la distancia euclídea, formando un nuevo cluster. 
```{r message= FALSE, warning=FALSE}
dist_mat <- dist(x, method = 'euclidean')#distancia euclidea entre todos los puntos
hclust_avg <- hclust(dist_mat, method = 'average')
plot(hclust_avg)#dendograma que muestra la jerarquía de clusters
```

Como hemos decidido que son 5 los cluster que queremos, podemos visualizar el árbol con dicho nivel de agregación.
```{r message= FALSE, warning=FALSE}
cut_avg <- cutree(hclust_avg, k = 5)#cortamos el arbol para ver los clusters
plot(hclust_avg)
rect.hclust(hclust_avg , k = 5, border = 2:6)
abline(h = 3, col = 'red')
```

Ploteamos en función de AnnualIncome y SpendingScore paara comparar con el k-means
```{r message= FALSE, warning=FALSE}
suppressPackageStartupMessages(library(dplyr))
x_cl <- mutate(x, cluster = cut_avg)
plot(x[c(2,3)], col=x_cl$cluster)
```

Aunque en un primer vistazo pueda parecer que rinde igual que el kmeans, el grupo del centro tiene más puntos que en anterior ejercicio. Esto es debido a que los centroides de los grupos de las esquinas se han ido agrupando al rededor de un núcleo que no es el más óptimo, en otras palabras, que está más alejado de un centro ideal (desconocido, pero intuido cuando visualizamos los datos), esto ha provocado que puntos que no pertenecen (a priori) al grupo del centro, hayan pasado a ese grupo al estar más alejados de lo que estaban en el kmeans de sus respectivos centroides.

Respecto al kmeans, varia en:
```{r message= FALSE, warning=FALSE}
table(x_cl$cluster,customers5clusters$cluster)
```

Explico esta matrix porque no queda muy claro debido a que algún grupo del kmeans corresponde a otro del Hierarchical y viceversa: en total hay 3+1+1+2=7 puntos que no coinciden, de 200 datos eso es una discrepancia del 3,5% con el kmeans.

* Density Based Clustering (DBSCAN): es un método no supervisado que, a diferencia del kmeans o del Hierarchical, funciona en situaciones de ruido, también funciona sin necesidad de indicar el número de clusters. Al contrario que en los anteriores, DSCAN utiliza la densidad de los datos para determinar los clusters y no la distancia euclídea. Las reglas del algoritmo son:
...
```{r message= FALSE, warning=FALSE}
library(dbscan)
db<-dbscan(x, eps = 0.08)
db
plot(x[c(2,3)], col=db$cluster)

```


Los parámetros del método pueden no ser los más óptimos aunque han sido elegidos mediante los criterios de la documentación. Como podemos ver, este método no funciona bien para este ejercicio, esto es debido a que, pese a que la implementación pudiera ser mejor, el método está pensado para aislar ruido de clusters con gran desidad de puntos. Es cierto que lo hace de forma excelente en el grupo del centro, pero de ninguna manera, este método detectaría los puntos que faltan a la derecha de la figura, pues la densidad en esas zonas es demasiado baja.
...
### Respuesta 2.1:
Comenzamos cargando el csv y realizando comprobaciones básicas.
```{r message= FALSE, warning=FALSE}
lastfm_df<-read.csv("lastfm.csv", header=T, sep=",")
attach(lastfm_df)
colSums(is.na(lastfm_df))
colSums(lastfm_df==" ")
summary(lastfm_df)
str(lastfm_df)
```
...
* Hay 19718 usuarios
* Los artistas más escuchados son radiohead, the beatles, coldplay, red hot chilly peppers, muse y metallica.
* El 74% de la reproducción de música es realizada por hombres.
* Estados Unidos es el país en el que más música se escucha, seguido de lejos por Reino Unido, Alemania, Polonia, Suecia y Brasil.

```{r message= FALSE, warning=FALSE}
lastfm_t<-read.transactions("lastfm.csv", header=T, sep=",")
inspect(head(lastfm_t,5))
```
Al inspeccionar el dataset vemos que tiene un listado de registros de las reproducciones de los usuarios.

Hacemos una primera comprobación 0.1 como valor de support:

```{r message= FALSE, warning=FALSE}
rules <- apriori(lastfm_t, parameter = list(support = 0.1, confidence = 0.1))
inspect(head(sort(rules, by = "confidence"), 45))
```
```{r message= FALSE, warning=FALSE}
inspect(head(sort(rules, by = "support"), 45))
```
```{r message= FALSE, warning=FALSE}
inspect(head(sort(rules, by = "lift"), 45))
```

El resultado es decepcionante, ¿Por qué ocurre esto?. Bien si estamos ante unos datos con información de multitud de usuarios de muchas naciones que escuchan muchos artistas diferentes, las únicas reglas de asociación que podemos extraer con un 10% de presencia global son las relatibas al género, pues no existe otro elemento de ningún conjunto que aparezca en el dataset con tanta frecuencia.

Si queremos sacar alguna conclusión interesenate de cara a implementar un sistema de recomendaciones, debemos bajar mucho la variable support. 

```{r message= FALSE, warning=FALSE}
rules2 <- apriori(lastfm_t, parameter = list(support = 0.001, confidence = 0.8))
inspect(head(sort(rules2, by = "confidence"), 20))
```

```{r message= FALSE, warning=FALSE}
inspect(head(sort(rules2, by = "support"), 20))
```

Esto nos da información respecto a los artistas, de nuevo asociada al sexo de los usuarios, en una dataset en el que estuviera igualado el número de mujeres y hombres esta información podría ser útil, pero no es el caso. Debemos aumentar el número mínimo de items si quieremos sacar algo más de provecho. Como dato curiosos hemos averiguado que el 95.7% de usarios de la india son hombres.

Variamos los valores para optener más información de los usuarios. Debemos bajar aún más el soporte y la confianza.
```{r message= FALSE, warning=FALSE}
rules3 <- apriori(lastfm_t, parameter = list(support = 0.0008, confidence = 0.5, minlen=3))
inspect(head(sort(rules3, by = "confidence"), 5))
```

```{r message= FALSE, warning=FALSE}
inspect(head(sort(rules3, by = "support"), 10))
```
```{r message= FALSE, warning=FALSE}
inspect(head(sort(rules3, by = "lift"), 6))
```

Estos datos nos proporcionan más infromación de los usuarios, pero aún podríamos sacar más, al menos a nivel teórico.

Para nuestra prueba vamos a intentar generar reglas que sugieran grupos a los usuarios en función de su sexo y país, la razón es que este el objetivo que más beneficio podría aportar desde una perspectiva de negocio. Probamos con radiohead, un grupo muy escuchado.
```{r message= FALSE, warning=FALSE}
rules4 <- apriori(lastfm_t, parameter = list(support = 0.00001, confidence = 0, minlen=3),appearance = list(rhs=c("radiohead")))
inspect(head(sort(rules4, by = "confidence"), 5))
```

El resultado son reglas para distintos conjuntos de personas, en una base de datos mayor o simplemente distinta, sería de esperar más información que pudiese propiciar a reglas con una mayor confianza. Por ejemplo si este ejercicio se hicera con datos de un servicio de streaming como Spotify, artistas como Drake o Dua Lipa saldrían en la columna de rhs con una confianza mucho más alta, en países como Estados Unidos.


Fuentes del ejercicio 1.1:
...
* Módulo 5
* Ejercicios de ejemplo PEC2

Fuentes del ejercicio 1.2:
...
* https://www.rdocumentation.org/packages/meanShiftR/versions/0.53/topics/meanShift
...
* https://stackoverflow.com/questions/6826320/graphing-results-of-dbscan-in-r
 
Fuentes del ejercicio 2.1:

* https://www.rdocumentation.org/packages/arules/versions/1.6-4/topics/transactions-class
* https://www.kaggle.com/aeryan/spotify-music-analysis
* https://www.rdocumentation.org/packages/arules/versions/1.6-4/topics/itemFrequencyPlot
